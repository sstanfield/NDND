...
static ndn::Block
make_rib_interest_parameter(const ndn::Name& route_name, int face_id)
{
    auto block = ndn::makeEmptyBlock(CONTROL_PARAMETERS);
    ndn::Block route_name_block = route_name.wireEncode();
    ndn::Block face_id_block = ndn::makeNonNegativeIntegerBlock(FACE_ID, face_id);
...
static ndn::Interest
prepareRibRegisterInterest(const ndn::Name& route_name, int face_id, ndn::KeyChain& keychain,
                           int cost = 0)
{
    ndn::Name name("/localhost/nfd/rib/register");
    ndn::Block control_params = make_rib_interest_parameter(route_name, face_id);
...
static ndn::Interest
prepareFaceCreationInterest(const std::string& uri, ndn::KeyChain& keychain)
{
...
static ndn::Interest
prepareFaceDestroyInterest(int face_id, ndn::KeyChain& keychain)
{
...
namespace ahnd {
    ...
    AHClient::AHClient(const Name &prefix, const Name &broadcast_prefix)
        ...
        m_multicast = std::make_unique<MulticastInterest>(m_face, m_controller, m_broadcast_prefix);
    }
    ...
    void AHClient::registerClientPrefix() {
        ...
        m_face.setInterestFilter(InterestFilter(name),
                                 ...
                                 [this](const Name &name) {
                                     std::cout << "AH Client: Registered client prefix "
                                               << name.toUri()
                                               << std::endl;
    ...
    void AHClient::registerArrivePrefix() {
        std::cout << "AH Client: Registering arrive prefix " << m_broadcast_prefix.toUri() << std::endl;
        m_arrivePrefixId =
                m_face.setInterestFilter(InterestFilter(m_broadcast_prefix),
                                         bind(&AHClient::onArriveInterest, this, _2, true),
                                         [this](const Name &name) {
                                             std::cout << "AH Client: Registered arrive prefix "
                                                       << name.toUri()
                                                       << std::endl;
        ...
        if (m_multicast->is_ready()) {
            ...
            m_multicast->expressInterest(interest,
                                         ...
                                         [](const Interest &interest) {
                                             ...
                                             std::cout
                                                     << "AH Client: Arrive Timeout (I am all alone?) "
                                                     << interest << std::endl;
                                         });
        } else {
            cout << "AH Client: Arrival Interest, multicast not ready will retry" << endl;
            m_scheduler->schedule(time::seconds(3), [this] {
                sendArrivalInterest();
            });
        }
...
// route information.
    void AHClient::onArriveInterest(const Interest &request, const bool send_back) {
        ...
        for (unsigned int i = 0; i < name.size(); i++) {
            ...
            if (ret) {
                ...
                auto ssStr = ss.str();
                std::cout << "AH Client: Arrival Name is " << prefix.toUri() << " from " << ssStr << std::endl;
                ...
                if (strcmp(ipStr, inet_ntoa(m_IP)) == 0) {
                    cout << "AH Client: My IP address returned - send back nothing" << endl;
                    continue;
                }
                DBEntry entry;
    ...
    void AHClient::registerRoute(const Name &route_name, int face_id,
                                 int cost, const bool send_data) {
        Interest interest = prepareRibRegisterInterest(route_name, face_id, m_keyChain, cost);
        m_face.expressInterest(
                interest,
                bind(&AHClient::onRegisterRouteDataReply, this, _1, _2, route_name, face_id, cost, send_data),
                ...
                [this, route_name, face_id, cost, send_data](const Interest &interest) {
                    std::cout
                            << "AH Client: Received timeout for interest "
                            << interest << std::endl;
        ...
        for (auto it = m_db.begin(); it != m_db.end(); it = m_db.erase(it)) {
            ...
            m_face.expressInterest(interest,
                                   [this, item](const Interest &interest, const Data &data) {
                                       cout
                                               << "AH Client: Arrive keep alive data "
                                               << interest.getName() << endl;
                                       m_db.push_back(item);
                                   },
                                   [this](const Interest &interest, const lp::Nack &nack) {
                                       // Humm, log this and retry...
                                       std::cout
                                               << "AH Client: received keep alive Nack with reason "
                                               << nack.getReason()
                                               << " for interest " << interest << std::endl;
    ...
    void AHClient::onRegisterRouteDataReply(const Interest &interest, const Data &data,
        ...
        if (response_code == OK) {
            ...
            if (send_data) {
                ...
                m_face.expressInterest(interest,
                                       [](const Interest &interest, const Data &data) {
                                           std::cout
                                                   << "AH Client: Record Updated/Confirmed from "
                                                   << data.getName()
                                                   << std::endl;
                                       },
                                       [this, route_name, face_id, cost, send_data](const Interest &interest,
                                           ...
                                           m_scheduler->schedule(time::seconds(3),
                                                                 [this, route_name, face_id, cost, send_data] {
                                                                     registerRoute(route_name, face_id, cost,
                                                                                   send_data);
                                                                 });
                                       },
                                       [this, route_name, face_id, cost, send_data](const Interest &interest) {
    ...
    void AHClient::onDestroyFaceDataReply(const Interest &interest, const Data &data) {
        short response_code;
        char response_text[1000] = {0};
    ...
    void AHClient::addFaceAndPrefix(const string &uri, const Name prefix, DBEntry entry, const bool send_data) {
        cout << "AH Client: Adding face: " << uri << endl;
        Interest interest = prepareFaceCreationInterest(uri, m_keyChain);
        m_face.expressInterest(
                interest,
                bind(&AHClient::onAddFaceDataReply, this, _1, _2, uri, prefix, entry, send_data),
                ...
                [this, uri, prefix, entry, send_data](const Interest &interest) {
                    std::cout
                            << "AH Client: Received timeout when adding face "
                            << interest << std::endl;
                    m_scheduler->schedule(time::seconds(3), [this, uri, prefix, entry, send_data] {
                        addFaceAndPrefix(uri, prefix, entry, send_data);
                    });
    ...
    void AHClient::destroyFace(int face_id) {
        ...
        m_face.expressInterest(
                interest,
                bind(&AHClient::onDestroyFaceDataReply, this, _1, _2),
                bind(&AHClient::onNack, this, _1, _2),
                bind(&AHClient::onTimeout, this, _1));
    }

    void AHClient::onSetStrategyDataReply(const Interest &interest, const Data &data) {
        Block response_block = data.getContent().blockFromValue();
        response_block.parse();
    ...
    void AHClient::setStrategy(const string &uri, const string &strategy) {
        ...
        m_face.expressInterest(
                interest,
                bind(&AHClient::onSetStrategyDataReply, this, _1, _2),
                bind(&AHClient::onNack, this, _1, _2),
                bind(&AHClient::onTimeout, this, _1));
    }
    ...
    void AHClient::setIP() {
        ...
        for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
            ...
            s = getnameinfo(ifa->ifa_netmask, sizeof(struct sockaddr_in), netmask, NI_MAXHOST, NULL, 0,
                            NI_NUMERICHOST);
